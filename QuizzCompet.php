...
<body class="quizzCompetition">
  ...
  ?>
    <form name="quizzCompetition" method="post" action="QuizzCompet_post.php">
      ...
      <br>
      <?php
      $query = $bdd->prepare("SELECT Reponse.libelleReponse, Reponse.isTrue FROM Reponse WHERE idQuestion= " . $result[$_SESSION['competition']][1]);
      $query->execute();
      $resultrsp = $query->fetchAll();
      $taille = count($resultrsp);
      for ($p = 0; $p < $taille; $p++) {
        echo ('<input type="radio" name="answer" value="' . $resultrsp[$p]['isTrue'] . '" required> ' . $resultrsp[$p]['libelleReponse'] . '<br>') ?><?php
                                                                                                                                            }
                                                                                                                                            echo ($_SESSION['isTrue']);
                                                                                                                                              ?>
      <br>
      <?php
      if ($_SESSION['competition'] == 11) {
      ?>
